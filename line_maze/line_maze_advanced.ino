...
#define NOTE_A4 440
//leftTurnCount remembers how many left turns it did, +1 for left turn, -1 for right turn. This is to handle cycles
int leftTurnCount = 0;
//number of left turns before we will make a right turn
...
void evaluateIntersection(){
  ...
  if(backleft){
    ...
    if(leftTurnCount>THRESHOLD && backright){
      leftTurnCount = 0;
      right();
    }
    left();
...
void left(){
  //update the counter by +1 for recording one more left turn
  leftTurnCount += 1;
  //get off the line
  turn(80);
  setAngularVel(angularSpeed);
...
void right(){
  //update the counter by -1 for recording one more right turn
  leftTurnCount -= 1;
  //make sure the turn counter doesn't go negative
  if(leftTurnCount<0) leftTurnCount = 0;
  //get off the line
  turn(-80);
  setAngularVel(-angularSpeed);
  //turn until we see the next line
...
void uTurn(){
  //update the counter by -2 for recording one more uturn
  leftTurnCount -= 2;
  //make sure the counter doesn't go negative
  if(leftTurnCount<0) leftTurnCount = 0;
  move(14);
  turn(160);
  setAngularVel(angularSpeed);
